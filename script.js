...
const knownImages = {
  ...
  "Tree Hut Tropic Glow Shea Sugar Scrub": "https://oletkaunis.fi/13592-medium_default/tree-hut-tropic-glow-shea-sugar-scrub-510g.jpg",
  "Sol de Janeiro Brazilian Bum Bum Cream": "https://m.media-amazon.com/images/I/61ino6WZGsL.jpg"
};
...
articleGrid.innerHTML = content.articles.map(item => `<article class="article-card"><a href="blog/${item.slug}.html"><div class="article-image ${item.color}"><span>${item.tag}</span><div class="article-art">${item.art}</div></div><p>${item.tag}</p><h3>${item.title}</h3><span>Read guide →</span></a></article>`).join('');
...
document.querySelector('#newsletterForm').addEventListener('submit', event => { event.preventDefault(); const form = event.currentTarget; const email = form.querySelector('#email').value.trim(); form.querySelector('.form-message').textContent = `You're on the list${email ? `, ${email}` : ''}. Watch your inbox for the next Find List.`; form.reset(); });
